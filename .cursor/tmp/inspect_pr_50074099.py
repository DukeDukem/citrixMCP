from playwright.sync_api import sync_playwright

CASE = "6a496cf84a3af12bb3b4957d"
with sync_playwright() as p:
    browser = p.chromium.connect_over_cdp("http://127.0.0.1:9222")
    target = None
    for page in browser.contexts[0].pages:
        if CASE in (page.url or ""):
            target = page
            break
    if not target:
        raise SystemExit("case tab not found")
    target.bring_to_front()
    body = target.inner_text("body")[:3000]
    print("---PAGE SNIP---")
    for line in body.splitlines():
        if any(k in line for k in ["Fall", "50074099", "Zahn", "Jörg", "Joerg", "Rechnung", "joerg"]):
            print(line[:220])
    frame = target.locator("[data-testid='baseEditorContainer'] iframe").first.content_frame
    text = frame.locator("body#tinymce").inner_text(timeout=5000) if frame else ""
    print("---EDITOR---")
    print("LEN", len(text))
    print("HAS_ANTWORT", "[Antwort]" in text)
    print("HAS_VERANLASST", "veranlasst" in text)
    print("HAS_WERKTAGE", "Werktagen" in text)
    print("HAS_JOERG", ("Jörg" in text) or ("Joerg" in text))
    print("FIRST_LINE", text.splitlines()[0] if text else "")
    print(text[:700])
