...
try:
    from fall_watch_state import (
        consume_fall_watch_prime,
        get_primed_info,
        is_fall_watch_primed,
        prime_fall_watch,
    )
except ImportError:
...
class EmailAutomation:
    ...
    def connect_to_browser(self, cdp_endpoint: Optional[str] = None, leave_page_unchanged: bool = False, stop_after_login: bool = False):
        ...
        if contexts and len(contexts) > 0:
            ...
            if pages and len(pages) > 0:
                ...
                chosen = self._pick_best_sprinklr_case_tab(pages, sprinklr_url_patterns)
    ...
    _FALL_H1_SELECTOR = 'h1[data-spaceweb="typography-h2"] span[data-testid="box"]'
    _CASE_TAB_URL_PATTERN = re.compile(r"/app/console/c/[a-z0-9]+")
    _GET_FALL_H1_JS = """
() => {
  const norm = (s) => (s || '').replace(/[\\u200f\\u200e]/g, '').replace(/\\u00a0/g, ' ').replace(/\\s+/g, ' ').trim();
  const span = document.querySelector('h1[data-spaceweb="typography-h2"] span[data-testid="box"]')
    || document.querySelector('h1 span[data-testid="box"]');
  if (!span) return '';
  const m = norm(span.textContent || '').match(/#?(\\d{6,})/);
  return m ? m[1] : '';
}
"""
    _GET_FALLNUMMER_SIDEBAR_JS = """
() => {
  const norm = (s) => (s || '').replace(/[\\u200f\\u200e]/g, '').replace(/\\u00a0/g, ' ').replace(/\\s+/g, ' ').trim();
  const field = document.querySelector('[data-entityid="Fallnummer"]');
  if (!field) return '';
  const html = field.querySelector('[data-testid="htmlText"]');
  const raw = html ? norm(html.textContent) : norm(field.textContent || '');
  const m = raw.match(/#?(\\d{6,})/);
  return m ? m[1] : '';
}
"""
    _COLLAPSED_CASE_ITEM_SELECTOR = 'button[data-testid="collapsed-case-item"]'
    _WAIT_COLLAPSED_CASE_CLICK_JS = """
() => new Promise((resolve) => {
  const cleanup = () => {
    if (window.__fallWatchClickHandler) {
      document.removeEventListener('mousedown', window.__fallWatchClickHandler, true);
      document.removeEventListener('click', window.__fallWatchClickHandler, true);
      window.__fallWatchClickHandler = null;
    }
  };
  const handler = (e) => {
    if (typeof e.button === 'number' && e.button !== 0) return;
    const btn = e.target.closest('button[data-testid="collapsed-case-item"]');
    if (!btn) return;
    cleanup();
    resolve({ ariaLabel: btn.getAttribute('aria-label') || '' });
  };
  window.__fallWatchClickHandler = handler;
  document.addEventListener('mousedown', handler, true);
  document.addEventListener('click', handler, true);
});
"""
    _REMOVE_COLLAPSED_CASE_CLICK_JS = """
() => {
  if (window.__fallWatchClickHandler) {
    document.removeEventListener('mousedown', window.__fallWatchClickHandler, true);
    document.removeEventListener('click', window.__fallWatchClickHandler, true);
...
"""
    ...
    def _pick_best_sprinklr_case_tab(self, pages, sprinklr_hosts: tuple[str, ...]):
        """Among open case tabs, prefer the focused tab; else the most recently listed."""
        case_tabs = []
        for p in pages:
            try:
                u = (p.url or "").lower()
                if u.startswith("devtools://"):
                    continue
                if any(host in u for host in sprinklr_hosts) and self._CASE_TAB_URL_PATTERN.search(u):
                    case_tabs.append(p)
            except Exception:
                continue
        if not case_tabs:
        ...
        for p in case_tabs:
            try:
                if p.evaluate("() => document.hasFocus()"):
                    logger.info("Selected focused Sprinklr case tab for RE/read-email")
                    return p
            except Exception:
                continue
        logger.info("Selected last Sprinklr case tab (no focused match among %s tabs)", len(case_tabs))
        return case_tabs[-1]

    def _get_case_id_from_sidebar_fallnummer(self) -> Optional[str]:
        """Fall # from Case Informationen sidebar (data-entityid=Fallnummer)."""
        try:
            raw = self.page.evaluate(self._GET_FALLNUMMER_SIDEBAR_JS)
    ...
    def _ensure_focused_case_page(self) -> None:
        """Re-attach self.page to the focused Sprinklr case tab (for Fall # watch)."""
        if not self.browser:
            return
        sprinklr_url_patterns = (
            "telefonica-germany.sprinklr.com",
        ...
        if chosen is None:
            for p in pages:
                u = (p.url or "").lower()
                if "sprinklr.com" in u and not u.startswith("devtools://"):
                    chosen = p
                    break
        if chosen is not None:
            self.page = chosen
    ...
    def _get_case_id_from_fall_h1(self) -> Optional[str]:
        """Fall # from visible h1 header: Fall <span data-testid=\"box\">#55893273</span>."""
        try:
            raw = self.page.evaluate(self._GET_FALL_H1_JS)
            if isinstance(raw, str) and raw.strip():
                digits = re.sub(r"\D", "", raw)
                if len(digits) >= 6:
                    return f"#{digits}"
            ...
            pass
        try:
            span = self.page.locator(self._FALL_H1_SELECTOR).first
            if span.count() and span.is_visible(timeout=800):
                return self.extract_case_id(span.inner_text())
        except Exception:
            pass
        return None

    def _get_visible_fall_id(self) -> Optional[str]:
        """Authoritative visible Fall # for watch/RE: h1 header, then sidebar, then legacy header."""
        fall_h1 = self._get_case_id_from_fall_h1()
        if fall_h1:
            return fall_h1
        return self._get_case_id_from_page_header()

    def _extract_current_case_for_re(self) -> tuple[str, dict] | tuple[None, None]:
        """Extract email content from the currently open case tab."""
        self._ensure_focused_case_page()
        case_id = self._get_visible_fall_id()
        if not case_id:
        ...
        try:
            ...
            logger.debug(f"Wait for email DOM during Fall watch: {e}")
        email_content = self.extract_email_content()
        if not email_content:
            email_content = {"body": "", "subject": "", "from": ""}
        final_id = email_content.get("case_id") or case_id
        if isinstance(final_id, str) and final_id.strip():
            digits = re.sub(r"\D", "", final_id)
            if digits:
                case_id = f"#{digits}"
        return case_id, email_content

    def _parse_fall_nr_from_aria_label(self, aria_label: str) -> Optional[str]:
        """Parse Fall # from sidebar button aria-label, e.g. 'Fall Nr. 55901259 von Andreas Doub'."""
        if not aria_label:
            return None
        match = re.search(r"Fall\s+Nr\.?\s*(\d{6,})", aria_label, re.I)
        if match:
            return f"#{match.group(1)}"
        return None

    def _normalize_fall_id(self, fall_id: Optional[str]) -> Optional[str]:
        if not fall_id:
            return None
        digits = re.sub(r"\D", "", fall_id)
        if len(digits) >= 6:
            return f"#{digits}"
        return None

    def _wait_for_collapsed_case_item_click(self) -> Optional[dict]:
        """Block until user left-clicks a sidebar collapsed-case-item button."""
        self._ensure_focused_case_page()
        try:
            self.page.evaluate(self._REMOVE_COLLAPSED_CASE_CLICK_JS)
        except Exception:
            pass
        try:
            result = self.page.evaluate(self._WAIT_COLLAPSED_CASE_CLICK_JS)
            if isinstance(result, dict):
                return result
        except KeyboardInterrupt:
            raise
        except Exception as e:
            logger.error(f"Wait for collapsed-case-item click failed: {e}")
            print(f"[ERROR] Case-item click listener failed: {e}")
        return None

    def _poll_until_fall_change_after_click(
        self,
        ...
        """Poll h1 Fall # until it changes to target or differs from baseline; then stop."""
        baseline_n = self._normalize_fall_id(baseline)
        target_n = self._normalize_fall_id(target)
        deadline = time.time() + max_seconds
        while time.time() < deadline:
            self._ensure_focused_case_page()
            current = self._normalize_fall_id(self._get_visible_fall_id())
            if current:
                if target_n and current == target_n:
                    return current
                if baseline_n and current != baseline_n:
                    return current
                if not baseline_n and not target_n:
                    return current
            time.sleep(check_interval)
        return None

    def monitor_fall_change_for_re(self, check_interval: int = 2) -> None:
        """
        Wait for manual left-click on sidebar case item (collapsed-case-item), then poll
        h1 Fall # until it changes. Extract once, print for 7-step RE, stop polling until
        the next case-item click.
        """
        logger.info("Starting click-gated Fall # watch (interval=%ss)", check_interval)
        print("\n" + "=" * 80)
        print("FALL # WATCH ARMED (click-gated + LF-primed auto-RE)")
        print("1. Finish current case: run LF or PR LF (primes next detection — does NOT start it)")
        print("2. Left-click next case in sidebar:")
        print('     button[data-testid="collapsed-case-item"]')
        print("3. Script polls h1 Fall # until change -> extract -> 7-step RE")
        print("Ctrl+C to stop.")
        print("=" * 80 + "\n")
        print("FALL_WATCH_ARMED")
        # Bootstrap: first case after session start (no prior LF yet)
        prime_fall_watch(case_id=None, source="session-start")
        print("FALL_WATCH_PRIMED source=session-start (first case only; thereafter use LF / PR LF)")
        ...
        while True:
            try:
                ...
                primed = is_fall_watch_primed()
                status = "primed" if primed else "NOT PRIMED — run LF or PR LF first"
                print(f"[{ts}] Waiting for case-item click… ({status})")
                click_info = self._wait_for_collapsed_case_item_click()
                if not click_info:
                    time.sleep(check_interval)
                    continue

                if not is_fall_watch_primed():
                    print("\n" + "=" * 80)
                    print("FALL_WATCH_NOT_PRIMED")
                    print("Run LF or PR LF on the finished case first, then click the next case.")
                    print("=" * 80 + "\n")
                    continue

                primed_info = get_primed_info()
                consume_fall_watch_prime()
                print(f"FALL_WATCH_PRIME_CONSUMED source={primed_info.get('source', '')} case={primed_info.get('case_id', '')}")

                aria_label = click_info.get("ariaLabel") or ""
                target_fall = self._parse_fall_nr_from_aria_label(aria_label)
                baseline = self._normalize_fall_id(self._get_visible_fall_id())

                print("\n" + "=" * 80)
                print("CASE_ITEM_CLICK_DETECTED")
                if aria_label:
                    print(f"aria-label: {aria_label}")
                if target_fall:
                    print(f"target Fall #: {target_fall}")
                print(f"baseline h1 Fall #: {baseline or '(none)'}")
                print("=" * 80 + "\n")

                current_id = self._poll_until_fall_change_after_click(
                    baseline=baseline,
                    target=target_fall,
                    check_interval=check_interval,
                )
                if not current_id:
                    print("[WARN] Fall # did not change within timeout - click another case or retry.")
                    continue

                if current_id == last_processed:
                    print(f"[INFO] Fall # {current_id} already processed this session - waiting for next click.")
                    continue

                prev = baseline or last_processed or "(none)"
                print("\n" + "=" * 80)
                print(f"FALL_CHANGE_DETECTED {current_id} previous={prev}")
                print("=" * 80 + "\n")

                case_id, email_content = self._extract_current_case_for_re()
                if case_id and email_content is not None:
                    self._print_customer_email_and_exit(case_id, email_content, watch_mode=True)
                    last_processed = self._normalize_fall_id(case_id) or current_id
                    print("FALL_DETECTION_STOPPED")
                    print("[INFO] Fall # polling stopped for this click. Waiting for next case-item click...\n")
                else:
                    print("[WARN] Fall # changed but email extract failed - waiting for next case-item click.")

            except KeyboardInterrupt:
                logger.info("Fall # watch stopped by user")
                try:
                    self.page.evaluate(self._REMOVE_COLLAPSED_CASE_CLICK_JS)
                except Exception:
                    pass
                print("\nFALL_WATCH_STOPPED")
                print("[INFO] Fall # watch stopped (Ctrl+C).")
                return
            except Exception as e:
                logger.error(f"Error in Fall # watch loop: {e}")
                import traceback
                traceback.print_exc()
                print(f"\n[ERROR] Fall # watch error: {e}")
                print("[INFO] Re-arming click listener after short delay...")
                time.sleep(check_interval)

    def _get_case_id_from_page_header(self) -> Optional[str]:
        """
        Get the case ID from the visible case: h1 Fall header, sidebar Fallnummer, then h2.
        Do not use page body or URL (avoids Kundennummer confusion).
        """
        fall_h1 = self._get_case_id_from_fall_h1()
        if fall_h1:
            return fall_h1
        sidebar_id = self._get_case_id_from_sidebar_fallnummer()
        if sidebar_id:
            return sidebar_id
        try:
...
""")
    ...
    def _print_customer_email_and_exit(self, case_id: str, email_content: dict, watch_mode: bool = False) -> None:
        ...
        print("="*80)
        if watch_mode:
            print("[INFO] Fall # watch: Cursor - run full 7-step RE for this case. Click next sidebar case when ready.")
        else:
...
def main():
    ...
    watch_fall_re = '--watch-fall-re' in sys.argv
    extract_once = '--extract-only' in sys.argv and not watch_fall_re
    ...
    try:
        ...
        # Skill 2: Fall # watch (auto-RE) or single extract
        if process_current_only and watch_fall_re:
            logger.info("Watch-fall-RE: monitoring h1 Fall # changes, extract on each change.")
            try:
                automation.monitor_fall_change_for_re(check_interval=max(2, CHECK_INTERVAL))
            finally:
                automation.cleanup()
            return
        ...
        if process_current_only:
            ...
            automation.process_current_page_once(chat_only=chat_only, extract_only=True)
            automation.cleanup()
            return
