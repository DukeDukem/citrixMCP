...
1b. **Attachments (EMAIL):** If Anhänge are present, open/download them **only so you understand the full case** (extra evidence beyond the email body). Prefer `open_case_attachments.py --view` (PDFs need View Detail on hover); `--download` → `C:\Users\PC ENTER\Downloads\yoummday temporaries`. Rule: `.cursor/rules/sprinklr-attachments.mdc`.
