...
**Language split (hard):** Address the **operator** in **English** in sections **1–5** and **7** (and all other agent chat). Section **6** and **PR** are **German only** (customer). See `.cursor/rules/agent-english-user-customer-german.mdc`.
...
| **5** | **Instructions on handling case** | **English.** Start from matrix Action; add KB steps that support it (ticket type/Themen-ID, inbox, Buchungsgrund, or "direct customer to X"); Sprinklr steps (create ticket, transfer, write reply). Cite matrix row + KB where relevant. **Never invent a transfer that the matrix forbids, or skip a transfer the matrix requires**, because a KB article says otherwise. **If transfer-eligible with a definitive Ziel-Kontakt, reduce to one clear transfer instruction.** |
| **6** | **Your response to customer** | **German only.** **Full** suggested email reply in **German**, copyable **code block** for **chat reading**: soft-wrap lines ~72 (max 80). **Opening:** one case-specific thank-you sentence only — **no** long *„Sie haben mitgeteilt, dass …“* recap (`.cursor/rules/reply-no-case-parroting.mdc`). Then actionable steps. **PR** = mail format (see `ENCODING-SAFE-OUTPUT.mdc`). Transfer-eligible: body may be minimal. |
| **7** | **Summary of response** | **English.** One short paragraph: what the reply says and what the agent/customer should do next. **For transfer-eligible cases with a definitive transfer goal, this can be a one-sentence note that the case was routed to \<Queue\> and no further Backoffice handling is performed.** |
...
2) **After script output (EMAIL only)** — **three-turn protocol** (see `.cursor/rules/re-no-background-tasks-ui.mdc`):
   - **Turn A:** extract Shell only; when stdout shows **`RE_TEXT_ONLY_GATE`**, stop (optional one-line ack max).
   - **Turn B:** sections **1–7** as **plain chat markdown** — **text-only message, ZERO tool calls** (no Grep, Task, Read terminals/*.txt, Write, play-ready, Auto-LF, arms).
   - **Turn C:** play-ready → Auto-LF → arms or wait for PR.
   Do **not** spawn explore/Task to read terminal capture files. Use CUSTOMER EMAIL from Shell stdout. Backup on disk: `.cursor/state/latest_re_visible.md` (hook) / `latest_extract.md` (extract).
...
**No case parroting (hard rule):** After salutation, **one thank-you sentence** acknowledges the issue; **do not** restate the customer's full story (*„Sie haben mitgeteilt …“*, multi-sentence recap). Go straight to solutions. Follow `.cursor/rules/reply-no-case-parroting.mdc`.
