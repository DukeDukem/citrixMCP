...
1. **Run the skill.** User types **RE** → bare `run.py` is **always `--once`** (extract currently open case). First case of the day / fresh agent = this push-start. **Never** arm Anwenden on typed RE. After **LF** / **PR LF**, use **`run.py --arm`** (detached) then **`run.py --await-arm`**. After **LF TR**, use **`--arm-weiter`** or **`--arm-extern`** then **`--await-arm`**. You output sections 1–7. Hook plays sound when section 7 is in your reply (or user types **`sound`**).
