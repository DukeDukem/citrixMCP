...
2) **After script output (EMAIL only)**, you MUST output sections **1–7 in chat** in order (see Mandatory output structure). This is the primary deliverable — Auto-LF / arms come **after**, never instead. Use the script output as the source for section 1. **Include date/time context in section 1.** Factor attachment content into verification / handling when relevant. **Do not** end the turn with only tools / LF / “armed” without the 7-step text.
