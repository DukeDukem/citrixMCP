...
| **3** | **Transfer eligibility** | **If verified:** Query **KnowledgeBase/TransferMatrix.md** **first** (authoritative). Identify Thema and Fall; state **Ziel-Kontakt** + **Action** (handling hint). **Transfer eligible: Yes** (other queue/email) or **No** (`CBC_CARE_ALLGEMEIN` / HANDLE DIRECTLY / Kein Transfer). **If KB routing conflicts with the matrix → matrix wins** (`.cursor/rules/transfer-matrix-priority.mdc`). |
| **4a** | **Transfer goal (if transferable)** | If Transfer eligible = Yes: state **Transfer goal** = matrix **Ziel-Kontakt** and **Action** (e.g. "Transfer in Sprinklr" or "Forward to email"). **Routing is final from the matrix** — keep remaining sections short; do not re-route from KB. |
| **4b** | **If not transferable** | If Transfer eligible = No: lead with the matrix **Action / handling hint** next to Ziel-Kontakt (HANDLE DIRECTLY instructions, Mein o2 paths, Themen-ID refs in the Action cell, Kein Transfer notes). **Then** query KB only for compatible process detail. Skip 4a. |
| **5** | **Instructions on handling case** | Start from matrix Action; add KB steps that support it (ticket type/Themen-ID, inbox, Buchungsgrund, or "direct customer to X"); Sprinklr steps (create ticket, transfer, write reply). Cite matrix row + KB where relevant. **Never invent a transfer that the matrix forbids, or skip a transfer the matrix requires**, because a KB article says otherwise. **If transfer-eligible with a definitive Ziel-Kontakt, reduce to one clear transfer instruction.** |
| **6** | **Your response to customer** | **Full** suggested email reply in **German**, copyable **code block** for **chat reading**: soft-wrap lines ~72 (max 80) so the user scrolls vertically only. **Wording/encoding** are the source of truth. **PR paste** must use **mail format** (previous paragraph length / signature layout) — do **not** carry chat soft-wraps into Sprinklr (see `ENCODING-SAFE-OUTPUT.mdc`). Do not truncate. **If transfer-eligible with a definitive transfer goal, body may be minimal** (forwarded to responsible team only). |
...
2) **After script output (EMAIL only)**, you MUST output sections **1–7 as visible chat markdown** in order (see Mandatory output structure). The user must see this text in the main chat thread **without** opening “finished background tasks” or any collapsed panel. This is the primary deliverable — Auto-LF / arms come **after** that visible text, never instead, never only inside tool/task UI. Prefer writing the full 7-step **before** invoking play-ready / fill_case_tracker / arm tools in the same turn (or a text-first turn, then tools).
