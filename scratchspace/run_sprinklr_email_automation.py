...
Sprinklr email automation script. Run this and nothing else.

  uv run python scratchspace/run_sprinklr_email_automation.py

Uses a dedicated Chrome profile (scratchspace/chrome_debug_profile) so Chrome
always binds to port 9222 even if your normal Chrome is already running.
"""
import os
import subprocess
import sys
...
# Dedicated profile so this Chrome instance always gets its own process and binds to 9222
...
def main() -> int:
    ...
    if not email_script.exists():
        ...
        return 1
    # Pass CDP so email_automation doesn't have to search; pass through --login-only and --process-current-only
    env = os.environ.copy()
