...
# Configure logging (log file in scratchspace when run from project root)
_script_dir = Path(__file__).resolve().parent
_repo_root = _script_dir.parent
_log_file = _repo_root / "scratchspace" / "email_automation.log"
logging.basicConfig(
    level=logging.INFO,
...
class EmailAutomation:
    ...
    def connect_to_browser(self, cdp_endpoint: Optional[str] = None):
        """
    ...
    def get_new_emails(self) -> List[Dict]:
        ...
        try:
            ...
            # First try the collapsed-case-item buttons (new emails in sidebar)
            email_selectors = [
                '[data-testid="collapsed-case-item"]',  # New email cases in sidebar
    ...
    def click_email_and_extract_content(self, email_data: Dict) -> Dict:
        ...
        if email_data.get('already_on_content_page', False):
            ...
            try:
                ...
                if email_data.get('element'):
                    ...
                    time.sleep(3)  # Wait for content page to load
                else:
        ...
        try:
            ...
            # Extract subject - look for "Betreff:" in the FIRST (newest) email
            # Get subject from the inbound message container
            subject_extracted = False
            try:
                # Try to get subject from the inbound message
                inbound_message = self.page.locator('[data-testid="inboundChatConversationItemFanMessage"]').first
                if inbound_message.is_visible(timeout=2000):
                    # Find subject label in this message
                    subject_label = inbound_message.locator('span[data-testid="label"]:has-text("Betreff:")').first
                    if subject_label.is_visible(timeout=1000):
                        parent = subject_label.locator('..')
                        # Get the second span (the one with the actual subject)
                        subject_spans = parent.locator('span[data-spaceweb="typography-bs2"]').all()
                        if len(subject_spans) >= 2:
                            subject_text = subject_spans[1]  # Second span has the subject
                            if subject_text.is_visible(timeout=500):
            ...
            # Extract from/sender - look for "Von:" in the FIRST (newest) email
            ...
            # Extract email body - based on actual HTML structure
            # There are multiple emails in the conversation, we want the FIRST one (newest/inbound)
            # The first email is in: [data-testid="inboundChatConversationItemFanMessage"] > ... > [data-testid="html-message-content"]
            body_extracted = False
            
            # Try to get the FIRST (newest) email - it's the inbound message
            try:
                # Find the inbound message container (customer's email)
                inbound_message = self.page.locator('[data-testid="inboundChatConversationItemFanMessage"]').first
                if inbound_message.is_visible(timeout=2000):
                    # Get the html-message-content from this specific message
                    body_elem = inbound_message.locator('[data-testid="html-message-content"]').first
                    if body_elem.is_visible(timeout=1000):
                        ...
                        try:
                            ...
                            pass
                        body_extracted = True
                        logger.info("Extracted body from inbound message")
                        print(f"[EXTRACTION] Body extracted from inbound message - length: {len(email_content['body'])} characters")
            except Exception as e:
            ...
            if not body_extracted:
                ...
                for selector in body_selectors:
                    try:
                        ...
                        if body_elem.is_visible(timeout=2000):
                            email_content['body'] = body_elem.inner_text()
                            try:
                                email_content['body_html'] = body_elem.inner_html()
                            except:
            ...
            if not body_extracted:
                try:
                    ...
                    if inner_div.is_visible(timeout=1000):
            ...
            if not body_extracted:
                try:
                    ...
                    if message_container.is_visible(timeout=1000):
            ...
            try:
                ...
                if attachment_elem.is_visible(timeout=1000):
                    ...
                    try:
                        if msg.is_visible(timeout=2000):
                            ...
                            try:
                                ...
                                if subject_label.is_visible(timeout=500):
                                    parent = subject_label.locator('..')
                                    subject_spans = parent.locator('span[data-spaceweb="typography-bs2"]').all()
                                    if len(subject_spans) >= 2:
                            ...
                            try:
                                ...
                                if from_label.is_visible(timeout=500):
                                    parent = from_label.locator('..')
                                    email_elem = parent.locator('span[data-spaceweb="typography-l2"]').first
                                    if email_elem.is_visible(timeout=500):
                            ...
                            try:
                                ...
                                if timestamp_elem.is_visible(timeout=500):
                    ...
                    try:
                        if msg.is_visible(timeout=2000):
                            ...
                            try:
                                ...
                                if to_label.is_visible(timeout=500):
            ...
            if conversation_messages:
                ...
                for msg_idx, msg in enumerate(reversed(conversation_messages), 1):
                    ...
                    if msg.get('body'):
                        ...
                        try:
                            ...
                            print(msg['body'].encode('utf-8', errors='replace').decode('utf-8', errors='replace'))
                        print("-" * 80)
                    else:
                ...
                if body_full and body_full != 'N/A':
                    try:
                        ...
                        print(body_full.encode('utf-8', errors='replace').decode('utf-8', errors='replace'))
                else:
...
""")
    ...
    def write_response_to_editor(self, response_text: str):
        ...
        try:
            ...
            # Method 2: Find iframe and write directly to body#tinymce
            if not editor_found:
                try:
                    logger.info("Attempting to find TinyMCE iframe...")
                    # Wait for TinyMCE editor container
                    tiny_mce_container = self.page.locator('[data-testid="tinyMCEEditor"]').first
                    if tiny_mce_container.is_visible(timeout=5000):
                        logger.info("Found TinyMCE container")
                        # Find the iframe - it has an ID ending with "_ifr"
                        iframe = tiny_mce_container.locator('iframe[id$="_ifr"]').first
                        if not iframe.is_visible(timeout=2000):
                            # Try any iframe in the container
                                    ...
                                    body.evaluate('''
                                        el => {
                                            ...
                                            if (window.parent && window.parent.tinymce) {
                                                try {
                                                    const editors = window.parent.tinymce.editors;
                                                    if (editors && editors.length > 0) {
                                                        editors[0].setContent(el.innerHTML);
                                                    }
                                                } catch (e) {
                                                    console.log('Could not notify parent TinyMCE:', e);
                                                }
                                            }
                                        }
                                    ''')
    ...
    def process_new_email(self, email_data: Dict):
        """
        Process a single new email: extract content, query Cursor AI, create output file, write response
        ...
        if is_transferable:
            ...
            if validation_errors:
                ...
                time.sleep(2)  # Wait for page to be stable and editor to be ready
                if not self.ensure_email_content_page():
                    ...
                    time.sleep(2)  # Wait after navigation
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                if current_state == 'console':
                    ...
                    time.sleep(2)  # Give it a moment to ensure we're really on console
                    return True
    ...
    def process_current_page_once(self) -> bool:
        """
        Process the current page once: if on email content page, extract and reply; if on console, process first visible email. Then exit (no monitoring).
        Used by Skill 2 (--process-current-only). Does not navigate; uses whatever tab is active.
        ...
        if current_state == 'console':
            ...
            if new_emails:
                self.process_new_email(new_emails[0])
                return True
                ...
                try:
                    ...
                    for email_data in new_emails:
                        try:
                            ...
                            time.sleep(2)  # Small delay after email processing
                            ...
                            try:
                                ...
                                pass
                            time.sleep(2)  # Small delay before continuing
...
def main():
    ...
    try:
        ...
        automation.connect_to_browser(CDP_ENDPOINT)
        ...
        # Skill 2 single-shot: process current page (email or first from list) then exit; do not navigate
        if process_current_only:
            logger.info("Process-current-only: processing current page once, then exiting.")
            automation.process_current_page_once()
            automation.cleanup()
            return
