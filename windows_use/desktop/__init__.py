...
class Desktop:
    def __init__(self, target_window_title: str | None = None):
        self.desktop_state = None
        self.target_window_title = target_window_title or os.environ.get("CITRIX_WINDOW_TITLE")
    ...
    def get_screenshot(self, scale: float = 0.7) -> Image.Image:
        ...
        if self.target_window_title:
            region = get_window_region_by_title(self.target_window_title)
            if region is None:
                logger.warning(
                    "Target window %r not found; falling back to full screenshot.",
    ...
    def get_capture_region_offset(self) -> tuple[int, int] | None:
        """
        When screenshots are taken from a target window (e.g. Citrix), returns (left, top)
        so that image coordinates (x, y) can be converted to screen coordinates (left+x, top+y).
        Returns None when capturing the full screen.
        """
        if getattr(self, "_last_capture_region", None) is not None:
            r = self._last_capture_region
            return (r[0], r[1])
        return None