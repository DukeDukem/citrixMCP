...
def _to_screen_coords(loc: tuple[int, int], desktop: Desktop | None) -> tuple[int, int]:
    """Convert possibly image-relative coords to screen coords when capturing a target window."""
    if desktop is None:
        return loc
    offset = desktop.get_capture_region_offset() if hasattr(desktop, "get_capture_region_offset") else None
    if offset is None:
        return loc
...
def click_tool(loc:tuple[int,int],button:Literal['left','right','middle']='left',clicks:int=1,desktop:Desktop=None)->str:
    ...
    loc = _to_screen_coords(loc, desktop)
    x,y=loc
    cursor.move_to(loc)
...
def drag_tool(from_loc:tuple[int,int],to_loc:tuple[int,int],desktop:Desktop=None)->str:
    ...
    from_loc = _to_screen_coords(from_loc, desktop)
    to_loc = _to_screen_coords(to_loc, desktop)
    control=desktop.get_element_under_cursor()
    x1,y1=from_loc
    x2,y2=to_loc
